 ...
 *
 *
 * Now, I'm not sure what that actually means as Perl is dual-licensed,
 * according to this: http://dev.perl.org/licenses/
 *
 * In conclusion, I'm calling it GPLv1.
 *
 */
...
int GetLength(metastring *s) { return s->length; }
...
void DoubleMetaphone(char *str, char **codes, int max_len) {
  int length;
  metastring *original;
  metastring *primary;
  ...
  while ((primary->length < max_len) || (secondary->length < max_len)) {
    ...
    case 'C':
      ...
      if (StringAt(original, current, 2, "CH", "")) {
        ...
        if ((StringAt(original, 0, 4, "VAN ", "VON ", "") ||
            ...
            || StringAt(original, (current - 2), 6, "ORCHES", "ARCHIT",
                        "ORCHID", "") ||
            StringAt(original, (current + 2), 1, "T", "S", "") ||
            ((StringAt(original, (current - 1), 1, "A", "O", "U", "E", "") ||
              (current == 0))
             /* e.g., 'wachtler', 'wechsler', but not 'tichner' */
             && StringAt(original, (current + 2), 1, "L", "R", "N", "M", "B",
                         "H", "F", "V", "W", " ", ""))) {
          MetaphAdd(primary, "K");
          MetaphAdd(secondary, "K");
        } else {
      ...
      if ((current == 0) && !StringAt(original, current, 4, "JOSE", "")) {
        ...
        if (IsVowel(original, current - 1) && !SlavoGermanic(original) &&
          ...
          if (current == last) {
            ...
            if (!StringAt(original, (current + 1), 1, "L", "T", "K", "S", "N",
                          "M", "B", "Z", "") &&
    ...
    case 'S':
      ...
      if (StringAt(original, current, 2, "SC", "")) {
        ...
        if (GetAt(original, current + 2) == 'H') {
          ...
          if (StringAt(original, (current + 3), 2, "OO", "ER", "EN", "UY", "ED",
                       "EM", "")) {
    ...
    case 'W':
      ...
      if (((current == last) && IsVowel(original, current - 1)) ||
          StringAt(original, (current - 1), 5, "EWSKI", "EWSKY", "OWSKI",
                   "OWSKY", "") ||
          StringAt(original, 0, 3, "SCH", "")) {
        MetaphAdd(primary, "");
        MetaphAdd(secondary, "F");
